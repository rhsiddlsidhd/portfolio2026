...
3.  **Locate Template**: Search for a pull request template.
    - Check the repository for:
      ...
      - If multiple templates exist in `.github/PULL_REQUEST_TEMPLATE/`,
        ask the user which one to use or select based on context (e.g., `bug_fix.md` vs `feature.md`).
    - If no template is found in the repository, use the default template from this skill's `references/pull_request_template.md`.

4.  **Read Template**: Read the content of the identified template file. Ensure you use the correct path from the `<available_resources>` section of this skill.
...
6.  **Preflight Check**: Check the project's configuration (e.g., `package.json`) for a preflight or validation script. If it exists, run it to ensure all checks pass.

    ```bash
    # Example (only if the script exists)
    npm run preflight
    ```

    If checks fail, address the issues before proceeding.
...
8.  **Create PR**: Use the `gh` CLI to create the PR.
    - **Verify Tool**: Check if `gh` is installed and authenticated (`gh auth status`). If not, inform the user.
    - **Draft & Create**: To avoid shell escaping issues with multi-line Markdown, write the description to a temporary file first.

    ```bash
    # 1. Write the drafted description to a temporary file (e.g., pr_body.md)
    # 2. Create the PR using the --body-file flag
    gh pr create --title "type(scope): succinct description" --body-file <temp_file_path>
    # 3. Remove the temporary file (Use OS-appropriate command: rm for Unix, Remove-Item for PowerShell)
    ```

    - **Title**: Ensure the title follows the
      [Conventional Commits](https://www.conventionalcommits.org/) format (e.g., `feat(ui): add new button`, `fix(core): resolve crash`).
